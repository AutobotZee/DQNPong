...
class DQN(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        self.conv1 = nn.Conv2d(3*num_scr, 16, kernel_size=5, stride=2)
...
def train(num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        state = torch.cat([s[i].squeeze() for i in range(num_scr)]).unsqueeze(0)
        if sum(d)!=0:
            ...
            for t in count():
                ...
                s[num_scr-1] = get_screen()
                if not done:
                    next_state = torch.cat([s[i].squeeze() for i in range(num_scr)]).unsqueeze(0)
                else:
                    next_state = None
